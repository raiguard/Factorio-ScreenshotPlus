...
local function setup_player(index)
    ...
    --     button_flow.add{type='sprite-button', name='ssp_editor_button', style=mod_gui.button_style, sprite='ssp-camera', tooltip={'gui-general.mod-gui-button-tooltip'}}.style.padding = 5
    -- end
end
...
local function create_movement_buttons(parent, name, side, opp)
    ...
    local flow = parent.add{type='flow', name='ssp_editor_pos_'..name..'_'..side..'_flow', direction=flow_direction}
    if flow_direction == 'vertical' then
        flow.style.vertical_spacing = 0
    else
        flow.style.horizontal_spacing = 0
    end
    local button_out = flow.add{type='sprite-button', name='ssp_editor_pos_'..name..'_'..side..'_out_button', style='close_button_light',
                                sprite='ssp-move-'..side..'-white', hovered_sprite='ssp-move-'..side, clicked_sprite='ssp-move-'..side}
    local button_in
    if opp then
        local side = opposite_sides[side]
        button_in = flow.add{type='sprite-button', name='ssp_editor_pos_'..name..'_'..side..'_out_button', style='close_button_light',
                                sprite='ssp-move-'..side..'-white', hovered_sprite='ssp-move-'..side, clicked_sprite='ssp-move-'..side}
    end
...
local function create_movement_pad(parent, name, sprite, opp)
    ...
    local table = flow.add{type='table', name='ssp_editor_pos_'..name..'_table', style='movement_pad_table', column_count=3}
    table.add{type='empty-widget', name='ssp_editor_pos_'..name..'_1'}
    create_movement_buttons(table, name, 'up', opp)
    table.add{type='empty-widget', name='ssp_editor_pos_'..name..'_3'}
    create_movement_buttons(table, name, 'left', opp)
    local button = table.add{type='sprite-button', name='ssp_editor_pos_'..name..'_edit_button', style='tool_button', sprite=sprite,
    ...
    table.add{type='empty-widget', name='ssp_editor_pos_'..name..'_7'}
...
local function create_setting_flow(parent, name, tooltip)
    ...
    flow.add{type='label', name='ssp_editor_settings_'..name..'_label', caption={'', {'gui-screenshot-editor.setting-'..name..'-label'}, tooltip and ' [img=info]' or ''},
...
event.register({defines.events.on_player_selected_area, defines.events.on_player_alt_selected_area}, function(e)
    ...
    filename_flow.add{type='textfield', name='ssp_editor_settings_filename_textfield', lose_focus_on_confirm=true, clear_and_focus_on_right_click=true}.style.width = 150
    local extension_flow = create_setting_flow(settings_frame, 'extension', true)
    extension_flow.add{type='drop-down', name='ssp_editor_settings_extension_dropdown', items={'.jpg', '.png', '.bmp'}, selected_index=1}.style.width = 70
    local quality_flow = create_setting_flow(settings_frame, 'quality', true)
    local quality_textfield = quality_flow.add{type='textfield', name='ssp_editor_settings_quality_textfield', text='100', lose_focus_on_confirm=true, clear_and_focus_on_right_click=true}
    ...
    local zoom_textfield = zoom_flow.add{type='textfield', name='ssp_editor_settings_zoom_textfield', text='1', lose_focus_on_confirm=true, clear_and_focus_on_right_click=true}
    ...
    checkboxes_flow.add{type='checkbox', name='ssp_editor_settings_alt_info_checkbox', caption={'gui-screenshot-editor.setting-alt-info-label'}, state=true}
    checkboxes_flow.add{type='empty-widget', name='ssp_editor_settings_checkboxes_filler', style='invisible_horizontal_filler'}
    checkboxes_flow.add{type='checkbox', name='ssp_editor_settings_antialias_checkbox', caption={'', {'gui-screenshot-editor.setting-antialias-label'}, ' [img=info]'},
    ...
    dialog_buttons_flow.add{type='button', name='ssp_editor_dialog_discard_button', style='back_button', caption={'gui.cancel'}}
    ...
    dialog_buttons_flow.add{type='button', name='ssp_editor_dialog_confirm_button', style='confirm_button', caption={'gui.confirm'}}
end)